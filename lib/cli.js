...
function acceptConnectionOptions(program) {
  program.option('-u, --username [username]', 'Salesforce username')
         ...
         .option('-f, --forceUrl [forceUrl]', 'Force URL in the form: force://clientId:clientSecret:refreshToken@instanceUrl')
...
function readOptions(program) {
  var options = {};
  "username,password,connection,loginUrl,forceUrl,pollInterval,pollTimeout".split(',').forEach(function(prop) {
